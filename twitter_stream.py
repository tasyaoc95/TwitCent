...
import clean_analyzer as ca #Important module which deals with loading pre trained

analyzer = SentimentIntensityAnalyzer()


ckey="QXfStPKBU7RpPZxdgki6XXTl1"
csecret="GHYusouK623ZsVftLYF4YYZAuMHBoLgnREMnKfUCXiEFvGnwRH"
atoken="1050727881222041600-vl8oMqSzd1lpNs2wN2miVw6XGlayOv"
asecret="wd9XPTgJUep8xVDbDYqKl0KXD7pYcFvnCiLYH6ehbY19G"
...
class listener(StreamListener):
    ...
    def on_data(self, data):
        try:
            ...
            #tweet = unidecode(data['text'])
            time_ms = data['timestamp_ms']
            clean_date_time = ca.clean_date(date_time)
            clean_text = ca.tweet_cleaner(tweet)
            neg_prob, pos_prob = ca.sentiment_score([clean_text])
            total_prob = (pos_prob-neg_prob)##
            print(clean_text, total_prob)
            sentiment = float(total_prob)
            #sentiment =1
